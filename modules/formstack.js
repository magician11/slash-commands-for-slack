...
function constructEndpointUrl(formId, searchFieldId, searchFieldValue) {
  return `${config.formstack
    .baseURL}/${formId}/submission.json?data=true&oauth_token=${config.formstack
    .token}&search_field_0=${searchFieldId}&search_value_0=${searchFieldValue}`;
}
  ...
  return new Promise((resolve, reject) => {
    request.get(
      ...
      (error, response, data) => {
        ...
        reject(
          `Could not find the option with ID ${fieldId} for channel ${channelName}.`
        );
      }
    );
  });
