...
module.exports = app => {
  ...
  app.post('/go', (req, res) => {
    ...
    const usage =
      "Usage: /go [time taken to assign] [person's name] [optional cc]";
    ...
    formstackSunbowl
      ...
      .then(trelloCardId => {
        trelloData.id = trelloCardId;
        return trelloSunbowl.findListId(assignee);
      })
      .then(trelloListId =>
        trelloSunbowl.moveTrelloCard(trelloData.id, trelloListId)
      )
      ...
      .then(taskList => {
        tasks = taskList;
        return formstackSunbowl.getFreshbooksProjectId(channel_name);
      })
      ...
      .then(billableHours => {
        ...
        return slackSunbowl.getFirstname(assignee.slice(1));
      })
      .then(firstName => {
...
If we have missed anything please let's us know by sending us a message in the <https://sunbowl.slack.com/messages/${channel_name}|#${channel_name}> channel.
...
This cycle took \`${timeTakenToAssign} hours\` to put together and assign out.
Bucket balance: \`${timeLeft.toFixed(1)} hours\``
        };

        slackSunbowl.postToSlack(goReviewMessage, response_url);
      })
      .catch(error => {
