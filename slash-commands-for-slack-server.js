...
if (environment === "production" || environment === "development") {
  ...
  https.createServer(sslOptions, app).listen(port, () => {
    console.log(
      `Slash commands for Sunbowl AI started listening on port ${port} [${environment} - https] at ${new Date().toString()}.`
    );
  });
} else {
  ...
  app.listen(port, () => {
    console.log(
      `Slash commands for Sunbowl AI started listening on port ${port} [${environment} - http] at ${new Date().toString()}.`
    );
  });
}
